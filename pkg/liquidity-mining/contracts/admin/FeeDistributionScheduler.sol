...
import "@balancer-labs/v2-interfaces/contracts/liquidity-mining/IFeeTokenDistributor.sol";
...
/**
 * @title FeeDistributionScheduler
 * @notice Scheduler for setting up permissionless distributions of fees to the feeDistributor.
 * @dev Any address may send tokens to the FeeDistributionScheduler to be distributed to the feeDistributor.
 */
contract FeeDistributionScheduler {
     ...
     * @param feeDistributor - The feeDistributor which is to distribute the reward token.
    ...
    function getRewardNode(
        IFeeTokenDistributor feeDistributor,
        IERC20 token,
        uint256 timestamp
    ) external view returns (RewardNode memory) {
        return _rewardsLists[_getRewardsListId(feeDistributor, token)][uint32(timestamp)];
    }
     ...
     * @param token - The token which is to be distributed among gauge depositors.
     */
    function getPendingRewards(IFeeTokenDistributor feeDistributor, IERC20 token) public view returns (uint256) {
        return getPendingRewardsAt(feeDistributor, token, block.timestamp);
    }
    ...
    function getPendingRewardsAt(
        ...
        mapping(uint32 => RewardNode) storage rewardsList = _rewardsLists[_getRewardsListId(feeDistributor, token)];
    ...
    function scheduleDistribution(
        ...
        _insertReward(_rewardsLists[_getRewardsListId(feeDistributor, token)], uint32(startTime), uint224(amount));
    }
     ...
     * @param token - The token which is to be distributed among gauge depositors.
     */
    function startDistributionForToken(IFeeTokenDistributor feeDistributor, IERC20 token) public {
        mapping(uint32 => RewardNode) storage rewardsList = _rewardsLists[_getRewardsListId(feeDistributor, token)];
        ...
        token.approve(address(feeDistributor), rewardAmount);
        feeDistributor.depositToken(token, rewardAmount);
    }


    // Internal functions

    function _getRewardsListId(IFeeTokenDistributor feeDistributor, IERC20 rewardToken) internal pure returns (bytes32) {
        return keccak256(abi.encodePacked(feeDistributor, rewardToken));
    }
